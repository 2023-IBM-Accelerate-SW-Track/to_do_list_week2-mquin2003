...
class Home extends Component {
  ...
  addTodo = (todo) => {
    ...
    if(this.state.todos.find(({ content }) => content === todo.content)){
      ...
      todo.id = Math.random();
      // Create a array that contains the current array (this.state.todos) and the new todo item (todo)
      let new_list = [...this.state.todos, todo];
      // console.log(this.state.todos.map(task => task.content));
      // console.log(this.state.todos.map(task => task.content).includes(todo.content));
